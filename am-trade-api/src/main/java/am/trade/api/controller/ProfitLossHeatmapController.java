...
public class ProfitLossHeatmapController {
    ...
    public ResponseEntity<ProfitLossHeatmapData> getYearlyHeatmap(
            ...
            @Parameter(description = "Whether to include trade details in the response") @RequestParam(defaultValue = "false") boolean includeTradeDetails) {

        try {
            log.info("Fetching yearly profit/loss heatmap for portfolio: {}", portfolioId);
            ProfitLossHeatmapData heatmapData = profitLossHeatmapService.getYearlyHeatmap(portfolioId,
                    includeTradeDetails);
    ...
    public ResponseEntity<ProfitLossHeatmapData> getMonthlyHeatmap(
            @Parameter(description = "Portfolio ID") @RequestParam String portfolioId,
            @Parameter(description = "Financial year (e.g., 2025 for FY 2025-26)") @RequestParam int financialYear,
            @Parameter(description = "Whether to include trade details in the response") @RequestParam(defaultValue = "false") boolean includeTradeDetails) {

        try {
            log.info("Fetching monthly profit/loss heatmap for portfolio: {} for financial year: {}", portfolioId,
                    financialYear);
            ProfitLossHeatmapData heatmapData = profitLossHeatmapService.getMonthlyHeatmap(portfolioId, financialYear,
                    includeTradeDetails);
            return ResponseEntity.ok(heatmapData);
        } catch (IllegalArgumentException e) {
            log.error("Invalid request parameters: {}", e.getMessage());
    ...
    public ResponseEntity<ProfitLossHeatmapData> getDailyHeatmap(
        ...
        try {
            ...
            ProfitLossHeatmapData heatmapData = profitLossHeatmapService.getDailyHeatmap(portfolioId, year, month,
                    includeTradeDetails);
            return ResponseEntity.ok(heatmapData);
        } catch (IllegalArgumentException e) {
            log.error("Invalid request parameters: {}", e.getMessage());
    ...
    public ResponseEntity<ProfitLossHeatmapData> getHeatmap(
            @Parameter(description = "Heatmap request parameters") @RequestBody HeatmapRequest request) {

        try {
            log.info("Fetching profit/loss heatmap for portfolios: {} with granularity: {}", request.getPortfolioIds(),
