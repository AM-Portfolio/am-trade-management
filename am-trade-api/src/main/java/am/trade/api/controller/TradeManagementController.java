 ...
 * Provides endpoints for retrieving trade details by various time periods and
 * portfolios
 */
@RestController
...
public class TradeManagementController {
        ...
        public ResponseEntity<Map<String, List<TradeDetails>>> getTradeDetailsByDay(
                        @Parameter(description = "Date to retrieve trade details for (format: yyyy-MM-dd)") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date,
                ...
                Map<String, List<TradeDetails>> tradeDetails = tradeManagementService.getTradeDetailsByDay(date,
                                portfolioId);
        ...
        public ResponseEntity<Map<String, List<TradeDetails>>> getTradeDetailsByMonth(
                ...
                Map<String, List<TradeDetails>> tradeDetails = tradeManagementService.getTradeDetailsByMonth(year,
                                month,
                                portfolioId);
                return ResponseEntity.ok(tradeDetails);
        }
        ...
        public ResponseEntity<Map<String, List<TradeDetails>>> getTradeDetailsByQuarter(
                ...
                log.info("Fetching trade details for year: {}, quarter: {} and portfolio: {}", year, quarter,
                                portfolioId);
                Map<String, List<TradeDetails>> tradeDetails = tradeManagementService.getTradeDetailsByQuarter(year,
                                quarter,
                                portfolioId);
                return ResponseEntity.ok(tradeDetails);
        }
        ...
        public ResponseEntity<Map<String, List<TradeDetails>>> getTradeDetailsByFinancialYear(
                ...
                Map<String, List<TradeDetails>> tradeDetails = tradeManagementService
                                .getTradeDetailsByFinancialYear(financialYear, portfolioId);
                return ResponseEntity.ok(tradeDetails);
        }
        ...
        public ResponseEntity<Map<String, List<TradeDetails>>> getTradeDetailsByDateRange(
                        @Parameter(description = "Start date of the range (format: yyyy-MM-dd)") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
                        @Parameter(description = "End date of the range (format: yyyy-MM-dd)") @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate,
                        @Parameter(description = "Optional portfolio ID to filter trades") @RequestParam(required = false) String portfolioId) {
                ...
                Map<String, List<TradeDetails>> tradeDetails = tradeManagementService.getTradeDetailsByDateRange(
                                startDate,
                                endDate, portfolioId);
                return ResponseEntity.ok(tradeDetails);
        }
        ...
        public ResponseEntity<Page<TradeDetails>> getTradeDetailsByPortfolio(
                        @Parameter(description = "Portfolio ID to retrieve trade details for") @PathVariable String portfolioId,
                        @Parameter(description = "Pagination information") Pageable pageable) {

                log.info("Fetching paginated trade details for portfolio: {}", portfolioId);
                Page<TradeDetails> tradeDetails = tradeManagementService.getTradeDetailsByPortfolio(portfolioId,
                                pageable);
                return ResponseEntity.ok(tradeDetails);
        }
}
