...
public class TradeJacksonModule extends SimpleModule {
    ...
    public TradeJacksonModule() {
        ...
        // DO NOT register list deserializer - let @JsonCreator handle it
        // The EnumLikeDeserializer will be used for individual enum instances
    }
    ...
    private static class EnumLikeDeserializer<T> extends StdDeserializer<T> {
        ...
        public T deserialize(JsonParser p, DeserializationContext ctxt) throws IOException {
            // Handle string format (old format or just code)
            if (p.currentToken() == JsonToken.VALUE_STRING) {
                ...
                try {
                    // Assume all enum-like classes have a fromCode(String, String) method
                    return (T) _valueClass.getMethod("fromCode", String.class, String.class).invoke(null, value, null);
                } catch (Exception e) {
                    throw new IOException("Failed to deserialize enum-like class from string: " + e.getMessage(), e);
                }
            }
            
            // Handle object format (new format with @JsonValue)
            if (p.currentToken() == JsonToken.START_OBJECT) {
                ...
                try {
                    return (T) _valueClass.getMethod("fromCode", String.class, String.class).invoke(null, code, description);
                } catch (Exception e) {
                    throw new IOException("Failed to deserialize enum-like class from object: " + e.getMessage(), e);
                }
            }
